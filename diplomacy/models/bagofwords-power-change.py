...
def convert_to_binary(dataset):
    ...
    for message in dataset:
        ...
        if POWER == "y":
            if message['game_score_change'] > 4:
                binary.append(1)
            else:
                binary.append(0)
...
def log_reg(train, test):
    ...
    newVec = CountVectorizer(tokenizer=spacy_tokenizer, vocabulary=vectorizer.vocabulary_, stop_words=STOP_WORDS,
                             strip_accents='unicode')
    if TASK == "SENDER":
        y = newVec.fit_transform([message['message'].lower() for message in aggregate(test)])
    elif TASK == "RECEIVER":  # for receivers, drop all missing annotations
        y = newVec.fit_transform([message['message'].lower() for message in aggregate(test) if
                                  message['receiver_annotation'] != "NOANNOTATION"])
...
if __name__ == '__main__':
    ...
    with jsonlines.open(data_path + 'train.power.jsonl', 'r') as reader:
        train = list(reader)
    # VAL NOT USED IN LOG REG FOR CONSISTENCY WITH NEURAL
    # with jsonlines.open(data_path+'validation.jsonl', 'r') as reade
    # dev = list(reader)
    with jsonlines.open(data_path + 'test.power.jsonl', 'r') as reader:
        test = list(reader)
